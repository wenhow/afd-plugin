...

{
  ...
  printf 'model_block_size=%s\n' "${MODEL_BLOCK_SIZE}"
  printf 'model_safetensors_load_strategy=%s\n' "${MODEL_SAFETENSORS_LOAD_STRATEGY}"
  printf 'kv_cache_dtype=%s\n' "${KV_CACHE_DTYPE}"
  printf 'model_speculative_method=%s\n' "${MODEL_SPECULATIVE_METHOD}"
  printf 'mode=%s/U%s\n' "${EXECUTION_MODE}" "${U_BATCHES}"
  printf 'async_scheduling=%s\n' "${AFD_ASYNC_SCHEDULING}"
  printf 'enable_mtp=%s\n' "${ENABLE_MTP}"
  printf 'hccl_ip=%s\n' "$(resolve_hccl_ip)"
