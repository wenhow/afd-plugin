...
PREFILL_DP_SIZE="${PREFILL_DP_SIZE:-8}"
...
case "${ENABLE_MTP}:${ENABLE_DSPARK}" in
  ...
  0:1)
    ...
    case "${EXECUTION_MODE}:${DSPARK_DRAFT_EXECUTION}" in
      eager:eager|full-decode-only:eager) DSPARK_DRAFT_ENFORCE_EAGER=true ;;
      full-decode-only:graph) DSPARK_DRAFT_ENFORCE_EAGER=false ;;
      *)
        echo "Unsupported DSpark target/draft execution: ${EXECUTION_MODE}/${DSPARK_DRAFT_EXECUTION}" >&2
        exit 2
        ;;
    esac
    DSPARK_CONFIG="$(printf '{"method":"%s","num_speculative_tokens":%s,"enforce_eager":%s,"draft_sample_method":"greedy"}' "$MODEL_SPECULATIVE_METHOD" "$DSPARK_NUM_SPECULATIVE_TOKENS" "$DSPARK_DRAFT_ENFORCE_EAGER")"
    MTP_ARGS=(--speculative-config "$DSPARK_CONFIG")
    ;;
  *)
