...
关闭两个新增门禁的本机范围，不替代单 A5 的 8 卡缩放矩阵、至少 12 卡环境的 A8F4、双机 PD 组合、完整 F1 或性能验收。

2026-09-11 双 A3 已完成第一阶段 A3 功能验收：A8F8 Graph/U2/MTP N2、N3 和
A4F8 Graph/U2/MTP N3 三个适用点各执行两轮，6/6 smoke、取消恢复、P1 128/128 和
...
时启动前 fail-fast 并将调度值写入证据。A5 同步调度复跑仍在相同 U2 capture 点卡住，
...
2026-09-16 A5 的 no-AFD DP4/MTP-off 已通过；A4F4 eager/U1、A4F4 Graph/U2、
A2F4 Graph/U2 和 A4F2 Graph/U2 四个 AFD 点均完成两轮 batch 1/8/32、取消恢复、
fatal、协调停机和 NPU 清理。三个 Graph/U2 点都固定 `async_scheduling=off`，每轮均
观测到真实 two-stage，早期 A5 Graph/U2 多流计算阻塞已关闭。A4F2 曾由 eager/U1
隔离和脱离 vLLM/模型/AFD 的两进程最小复现收敛为 rank-size 2 BF16 ReduceScatter
...
和强杀门禁不干净，作为启停工具问题继续记录，不影响 2026-09-11 另行确认的第一阶段
A3 功能范围。采集时 R14 overlay
...
A3 当前阶段
  ...
  -> MTP eager 非等量协议/组件闭环（M5 已完成，A8F4 实模转至少 12 卡环境）
  ...
  -> A8F4 实模容量预检（A3 EP4 HBM 不足；单 A5 仅 8 卡，转多节点）
...
第一阶段交付 DeepSeek-V4 AFD 的 A3 功能，不包含 U3、逐 token 精度和正式性能收益，也不把启停辅助脚本质量作为交付目标。执行口径固定 TP1；TP/SP/CP/DCP/PP、TP3、非等量 TP2 和 TP2 最大 Graph+MTP 组合不作为第一阶段目标。
...
本机 CPU/Mock、NPU 组件和实模 smoke（M10/M11 已完成）
  -> 双 A3：PD + Graph/U2 + MTP 多 token、A8F8 N2/N3 与 A4F8 N3 各两轮（已完成）
  -> 第一阶段 A3 功能 tag（已冻结）

后续功能/精度范围
  单 A5 8 卡：MTP-off A4F4/A4F2/A2F4 实模（已完成）
...
2026-09-08 已补齐外部验证交付物；2026-09-11 双 A3 的一期功能硬件证据已完成；2026-09-12 A5 进入独立 bring-up，并于 2026-09-16 完成 standalone Decode-AF：
...
- A5 standalone AF 子阶段固定 MTP-off：用 NPU 0-3、DP4 启动一次 no-AFD 服务；该基线已完成，
  ...
  golden。no-AFD 和 4 个 AFD 点均已通过；更新 HCCL 环境后的 A4F2 两轮通过，
  `A5-HCCL-RS-001` 在当前环境关闭。
  这些结果只覆盖 Decode-AF，未覆盖 A5 PD。
- 2026-09-14 的历史现场结果中，DP4 no-AFD/MTP-off 与 A4F4 eager/U1 两轮完整门禁已通过。A4F4
...
- 双 A3 原始证据分析已经关闭第一阶段 A3 功能范围并创建 `dsv4-afd-v023-phase1-a3-functional-v1`；A5 当前独立执行无 golden 功能 smoke，路径匹配 F1 和正式性能保持后续项。
...
真实 NPU 组件闭环；A4F8 eager/U1/N2 实模 smoke 已通过。单 A5 的 fan-in 候选缩放为
A4F2；完整 A8F4 需要至少 12 张可用 NPU，不再列为单 A5 目标。

Mooncake PD 的 split A16F8 也是物理 `A:F=2:1`，但它采用 P8F8 + A16、TP1，不能替代
standalone A8F4/EP4 的多节点 E2E 门禁。该双机点还含 R14 历史 overlay，只能作为运行和
...
| A3-P6 | A8F4 eager/U1、U2 E2E 正确性 | A3 停止：EP4 模型构造 HBM 不足；A10F5 被固定栈 EP5 专家放置拒绝；单 A5 仅 8 卡，完整 A8F4 E2E 转至少 12 卡环境 |
...
| A3-P7M5 | eager 非等量拓扑 + MTP | 已通过 A1F1/A2F1/A4F2 真实 NPU 组件；单 A5 改测 A4F2/A2F4，完整 A8F4 留到至少 12 卡环境 |
| A3-P7M6 | Graph 非等量拓扑 | 已通过 graph key 隔离、A2F1/A4F2 两 stage capture/replay 和 A4F2 target Graph + eager MTP 组合组件；单 A5 改测缩放拓扑，完整 A8F4 F0 留到至少 12 卡环境 |
...
| A3-P7M9 | Mooncake PD + AFD | TP1/Graph U2 双 A3 数据面已完成；一期 A8F8 N2/N3、A4F8 N3 各两轮，6/6 smoke、取消恢复、P1 和全 rank 在线 U2 通过，A3 功能目标 3/3 关闭；逐 token F1、启停脚本和正式 P2 后移 |
| A3-P7M10 | 多 speculative token | 已冻结：保持单 MTP layer，对外最大 `N=3`；`N=1/2/3` 代码回归、16 项 NPU 组件和 A8F8 N2/N3 双机两轮功能通过；A5 的 5 个路径 control 与 9 点 F1 是后续精度范围 |
| A3-P7M11 | `F=kA` 双向非等量拓扑 | 已冻结第一阶段 A3 代表点：双向整数 rank mapping、scatter/gather、dummy padding、控制面、MTP/Graph 通过；A4F8 N3 双机两轮功能通过。单 A5 8 卡缩放矩阵和完整 A8F4 仍为后续范围 |
...
因此 A3-P6 的结论是“受硬件/固定栈容量阻塞”，不是 connector 正确性失败。A3 不再尝试用超卖或改变专家放置强行完成 A8F4；单 A5 只有 8 卡也不能承载该 12 卡拓扑，该 E2E 门禁保留给至少 12 张可用 NPU 且 HBM 足够的环境。

### 5.9 A3-P7：等量/非等量联合 profiling 和调优

当前 A3 实际执行范围收敛为 A8F8；A8F4 联合 profile 等至少 12 卡环境完成实模加载后再恢复。第一阶段仅做同步 HCCL 优化：
...
```text
Topology: A8F8（双 A3）；A4F4/A4F2/A2F4（单 A5 8 卡）；A8F4（至少 12 卡环境）
Mode: U1, U2
...
FFN EP4 专家权重峰值 HBM 不足，而不是 connector 协议未通过。单 A5 只有 8 卡，先跑
A4F4/A4F2/A2F4 并生成同平台原生 MTP golden；完整 A8F4 转至少 12 卡环境，再决定是否
进入 A8F4 P1/P2。因此 M5 只允许创建
...
30/30 golden、batch 1/8/32、生命周期或 P1/P2 结论；单 A5 只有 8 卡，缩放到
A4F2/A2F4 验证，完整 A8F4 门禁保留到至少 12 卡且 HBM 足够的环境。
...
M7 只创建功能 tag，不创建性能 tag；A8F4 实模 full-draft Graph 继续留到至少 12 卡环境。
...
- 驱动、固件、CANN、HCCL 和 torch-npu 版本；HCCL 必须保存精确包名、build 和来源，不能只写“最新包”；
- A5 对应 ops 包；
...
现场替换 HCCL 包后，A4F2 Graph/U2/MTP-off 也于 2026-09-16 正式两轮通过，
`A5-HCCL-RS-001` 在当前运行环境关闭。证据包
`689a607d8f1043c0961395a5bf2596bc.gz` 的 SHA256 为
`5447a2a6999282777f0ded60f61eb190284194bad831a72720a2d82ec1f3cab0`：两个 cycle
均通过 batch 1/8/32、取消与请求归零、恢复、真实 two-stage、fatal、协调停服和 NPU
清理门禁，启动耗时分别为 144.045 秒和 120.046 秒。运行栈记录为 CANN 9.2.0、
`npu-smi 25.6.rc1.b188`、vLLM `0fc695fc`、vLLM-Ascend `3da28f941`、
afd-plugin `c0e030f` 加 tracked diff SHA256
`fb59a1708f9723ae314cff20d191640ebe594454950cbd63365bef143031bb6e`。

本次归档未记录独立 HCCL 包的精确包名/version/build，afd-plugin 也不是 clean tree，
因此可以确认旧 `507018`/`PreSyncInterThreads` 故障在当前组合不再复现，并关闭当前
功能阻塞，但不能把修复精确归因到某个 HCCL build。FFN 受控停服尾部的
`KeyboardInterrupt: terminated`/`ERR99999` 在双方返回码为 0、shutdown/fatal/cleanup
...
现场 A5 单机已确认为 8 个 NPU；A4F4、A2F4 和 A4F2 的当前 MTP-off 功能门禁均已完成，除运行栈或代码变化后的定向回归外不再重跑。A8F8 需要 16 卡，A8F4/A4F8 需要 12 卡，均不在单 A5 执行。非等量候选必须满足较大侧是较小侧的整数倍。实际角色映射必须根据 `npu-smi` 拓扑和 NUMA/NIC 关系决定，不能只按 device ordinal 对半切分，也不能在未测 HBM 前假定更少 FFN rank 一定可行。
...
15. M9 Mooncake PD 的第一阶段 A3 功能里程碑已关闭：A8F8 N2/N3、A4F8 N3 各两轮，6/6 smoke、取消恢复、P1 和全 rank 在线 U2 通过，并由 `dsv4-afd-v023-phase1-a3-functional-v1` 冻结。早期 TP1/MTP off/Graph U2 的三拓扑三轮性能和双侧 DP0 Profile 仍仅作观测；A16F8 是物理 2:1，按 wall 归一化后 FFN Free 从 71.977% 降至 38.395%，Bubble 却从 18.164% 升至 36.954%，不能宣称 bubble 已消除。R14 AllToAllV 静态 split-cache 已撤回。动态路由扩展、启停脚本、路径匹配 PD control、30/30 F1、C0 加 T1/T2/T3 x all-on/V1/off 的 10 单元正式 P2 和固定 F16 的物理 1:1/2:1/4:1 扫描均为后续项；A64F16 超出当前双 16 卡 A3，不能从 A16F8 外推；
